...
### 5. (Опционально) Дублировать критичные события в наш backend (`ANALYTICS_SPEC.md §5`)
Не обязательно, но желательно для быстрого дебага/своей копии:
- `POST https://<fairy-host>:3000/api/analytics/event`, без авторизации, fire-and-forget, батч ≤ 50.
- Дублировать минимум: `purchase_*`, `paywall_view`, `tale_complete`. Полный поток `tale_page_view`
  сюда НЕ слать (объём — для глубины чтения есть BigQuery).
